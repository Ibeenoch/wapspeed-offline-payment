  ...
  return (
        <View style={className`flex-1 px-4 ${currentMode === 'light' ? 'bg-[#f7f7f7]' : 'bg-[#000e28]'}`}>
            <View style={className`flex-row justify-center items-center h-[90%] gap-3 px-8`}>
                ...
                <Text style={className`${getmode.text} text-sm w-[250px] `}>Payment Processing for {qrCodedetails['Merchant Name']} for the payment of ₦{qrCodedetails.Amount} to {qrCodedetails['Account Number']}, You will receive an SMS notification upon completion.</Text>
            </View>
